function [ regions, regionStats ] = getMSERRegionsAllParams( IMG_Original, sigmaSpatial,sigmaRange , gamma ,T, Delta)
    ...
    v_retinex = retinexFilter(Gray, sigmaSpatial, sigmaRange, samplingSpatial, samplingRange, gamma, 0);

    %Take MSER
    [regions, mserConnComp] = detectMSERFeatures(v_retinex, 'ThresholdDelta', Delta*100, 'MaxAreaVariation', T);
    
    %Get RegionStats
    
    regionStats = regionprops(mserConnComp,'PixelIdxList','Centroid','MajorAxisLength', 'MinorAxisLength','Orientation', 'Area', 'BoundingBox', 'Euler', 'Solidity', 'Extent', 'Image');
    ...
    for N =1:length(regionStats)
       BB =  [regionStats(N).BoundingBox];
       BBall = [regionStats(:).BoundingBox];
       ...
       [~, I] = max([regionStats(Overlap).Area]);
    ...
    regionStats = regionStats(Keep);
    
    
    %% Get Hue Stat
    for i= 1:length(regions)
       regionStats(i).Hue = Hue([regionStats(i).PixelIdxList ]);
    end
