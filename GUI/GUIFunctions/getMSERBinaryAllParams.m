function [ MSERRegions_Binary, BinarisedRegionStats ] = getMSERBinaryAllParams( IMG_Original, sigmaSpatial,sigmaRange , gamma, k, alpha )
    global HEIGHT
    ...
    v_retinex = adapthisteq(v_retinex);

    
    %Get binary CC
    IMG_Binarised = sauvola(v_retinex, floor(HEIGHT*alpha),k);
    [MSERRegions_Binary, mserConnComp] = detectMSERFeatures(IMG_Binarised);

    BinarisedRegionStats = regionprops(mserConnComp,'PixelIdxList','Centroid','MajorAxisLength', 'MinorAxisLength','Orientation', 'Area', 'BoundingBox', 'Euler', 'Solidity', 'Extent', 'Image');

    % Get Hue Stat
    for i= 1:length(BinarisedRegionStats)
       BinarisedRegionStats(i).Hue = Hue([BinarisedRegionStats(i).PixelIdxList ]);
    end
